...
case class Alliance(
  ...
  val status = {
    if (
      members
        ...
        .length < Alliance.ESTABLISH_MIN_MEMBER_COUNT
    ) {
  ...
  def removeMember(member: AllianceMember, removeSelf: Boolean): Unit = {
    ...
    if (members.find(_.airline.id == targetAirlineId).isDefined) { // just to play safe
      ...
      if (removeSelf) {
        AllianceSource.saveAllianceHistory(
          AllianceHistory(
            allianceName = name,
            airline = member.airline,
            event = LEAVE_ALLIANCE,
            cycle = currentCycle
          )
        )
        if (member.role == LEADER) {
          members
            .filter(_.role == CO_LEADER)
            .sortBy(_.joinedCycle)
            .headOption match {
            case Some(coLeader) =>
              AllianceSource.saveAllianceMember(
                coLeader.copy(role = AllianceRole.LEADER)
              )
              AllianceSource.saveAllianceHistory(
                AllianceHistory(
                  allianceName = name,
                  airline = coLeader.airline,
                  event = PROMOTE_LEADER,
                  cycle = currentCycle
                )
              )
            case None =>
              AllianceSource.deleteAlliance(
                id
              ) // no co-leader, remove the alliance
          }
        }
      } else {
...
case class AllianceMember(
    allianceId: Int,
    airline: Airline,
    role: AllianceRole.Value,
    joinedCycle: Int
)
...
object AllianceRole extends Enumeration {
  ...
  val isAdmin: AllianceRole => Boolean = (role: AllianceRole) =>
    role match {
      case LEADER | CO_LEADER => true
      case _                  => false
    }

  val isAccepted: AllianceRole => Boolean = (role: AllianceRole) =>
    role match {
...
case class AllianceHistory(
    allianceName: String,
    airline: Airline,
    event: AllianceEvent.Value,
    cycle: Int,
    var id: Int = 0
)
...
object AllianceEvent extends Enumeration {
  ...
  val FOUND_ALLIANCE, APPLY_ALLIANCE, JOIN_ALLIANCE, REJECT_ALLIANCE,
      LEAVE_ALLIANCE, BOOT_ALLIANCE, PROMOTE_LEADER, PROMOTE_CO_LEADER, DEMOTE =
    Value
}

object Alliance {
  val MAX_MEMBER_NON_REGIONAL_COUNT = 8
  val MAX_MEMBER_REGIONALS_COUNT = 8
  ...
  /** @param alliances
    * @return
    *   Map[Alliance, (ranking, champion points)] . Take note that ranking
    *   starts with 1 as the top alliance
    */
  def getRankings(
      alliances: List[Alliance]
  ): Map[Alliance, (Int, BigDecimal)] = {
    // val countryChampions = ChampionUtil.getAllCountryChampionInfo()
    val airportChampionsByAirlineId =
      ChampionUtil.loadAirportChampionInfo().groupBy(_.loyalist.airline.id)
    val alliancesWithChampionPoints: List[(Alliance, BigDecimal)] =
      alliances.filter(_.status == AllianceStatus.ESTABLISHED).map { alliance =>
        ...
        alliance.members.foreach { allianceMember =>
          val memberChampionPoints: BigDecimal =
            if (allianceMember.role == AllianceRole.APPLICANT) { // do not add champion points from applicant
              ...
              BigDecimal(
                airportChampionsByAirlineId
                  .get(allianceMember.airline.id)
                  .map(_.map(_.reputationBoost).sum)
                  .getOrElse(0.0)
              )
            }
    ...
    val alliancesWithRanking = alliancesWithChampionPoints
      .sortBy(_._2)(Ordering.BigDecimal.reverse)
      .zipWithIndex
      .map { case ((alliance, allianceChampionPoints), sortedIndex) =>
        (alliance, (sortedIndex + 1, allianceChampionPoints))
      }
